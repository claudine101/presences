...
class Historique_Commentaire extends CI_Controller
{
	...
	function listing()
	{
		...
		$order_column = array('COMMENTAIRE_TEXT','VERIFICATION');
	...
	function update()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->getOne($this->input->post('COMMENTAIRE_ID'));
		} else {
			...
			$this->Modele->update('historique_commentaire', array('COMMENTAIRE_ID' => $this->input->post('COMMENTAIRE_ID')), $data);
