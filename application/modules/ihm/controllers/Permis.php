...
class Permis extends CI_Controller
{
     ...
     function get_donne_permis($id = null)
    {

        $Permis=$this->Modele->getRequeteOne('SELECT `NUMERO_PERMIS`,`NOM_PROPRIETAIRE`,`CATEGORIES`,DATE_NAISSANCE,`DATE_DELIVER`,`DATE_EXPIRATION`,`POINTS` as POINT_PERMIS FROM `chauffeur_permis_tampos` WHERE `NUMERO_PERMIS`="'.strtoupper($id).'" ORDER BY `ID_PERMIS` DESC LIMIT 1 ');


        // '".$Permis['NUMERO_PERMIS']."'

        $rapport = $this->Modele->getRequete("SELECT SUM(q.MONTANT) AS AMENDE_PERMIS, SUM(q.POURCENTAGE) AS POINT_PERMIS, q.INFRACTIONS,COUNT(q.ID_CONTROLES_QUESTIONNAIRES) as nbr FROM historiques h JOIN autres_controles co ON h.ID_COMPORTEMENT = co.ID_CONTROLE_PLAQUE JOIN autres_controles_questionnaires q ON co.ID_CONTROLES_QUESTIONNAIRES = q.ID_CONTROLES_QUESTIONNAIRES WHERE 1 AND `HAS_PERMIS` = 1 AND h.`NUMERO_PERMIS` = '".$Permis['NUMERO_PERMIS']."' GROUP BY q.INFRACTIONS");
        ...
        if (!empty($rapport)) {
            ...
            foreach ($rapport as  $value) 
            {
                ...
                $date  =  !empty($value['INFRACTIONS']) ? $value['INFRACTIONS'] : '';
                $catego.= "'".$date."',";
                $Points .=  $nn.",";
                $Amendes .=  $mm.",";
                $Nfois .=  $nbr.",";
        ...
        data: [".$Nfois."]
            ...
            $data['totalInter'] = $nombreFois;

            $data['title']= "Permis Utilisateur";
            $data['Permis']=$Permis;
            // $data=$this->load->view('Permis_View',$data);

            $map = $this->load->view('Permis_View_get', $data, TRUE);
            $output = array('donne_permis' => $map);
