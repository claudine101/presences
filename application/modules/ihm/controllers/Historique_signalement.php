...
class  Historique_signalement extends CI_Controller
{
  function index()
  {
    $data['nombre']=$this->Model->getRequeteOne('SELECT COUNT(`ID_HISTO_SIGNALEMMENT`) AS NBR FROM `historique_signalement` WHERE `VALIDATION`=0');
  ...
  public function valideer_multiple()
  {
    $listes_id = $this->input->post('liste_id');

   

    foreach ($listes_id as $val) {
   ...
   $stat = 4;
    ...
    $this->sendVerbalisationMessage($val);
  ...
  function validations($id = 0, $idhis = 0)
  {
    ...
    $sms = $data_update['NOM_PROPRIETAIRE'] . ",votre vehicule " . $data_update['MARQUE_VOITURE'] . " plaque " . $data_update['NUMERO_PLAQUE'] . " vient d'être verbalisé par la PNB  amende : " . number_format($data_update['AMENDE'], 0, ',', " ") . " FBU a payer dans un delais de 72h";
  ...
  function listing()
  {
    ...
    if(!empty($DATE_DEBUT) && !empty($DATE_FIN)){
      $date_interval= ' AND hs.DATE_INSERT between "'.$DATE_DEBUT.'" AND "'.$DATE_FIN.'"';
    ...
    $query_principal = 'SELECT ID_HISTO_SIGNALEMMENT,pr.STATUT, obr.NUMERO_PLAQUE,hs.DATE_INSERT,ca.STATUT_TRAITEMENT,sv.DESCRIPTION, DESCRIPTION_INFRATION, AMENDE,sv.ID_STATUT,STATUT_PAYE, IF(STATUT_PAYE=1, "payé" ,"non payé") AS STATUT_PAY ,hs.VALIDATION, DATE_INSERT,sv.DESCRIPTION as STATUT_TRAITEM,sv.ID_STATUT as status, ca.CIVIL_DESCRIPTION,ca.IMAGE_1,ca.IMAGE_2,ca.IMAGE_3, user.NOM_UTILISATEUR,COMMENATAIRE FROM historique_signalement hs LEFT JOIN obr_immatriculations_voitures obr on obr.ID_IMMATRICULATION=hs.ID_IMMATICULATION LEFT JOIN utilisateurs user ON user.ID_UTILISATEUR=hs.USER_ID LEFT JOIN profil pr ON pr.PROFIL_ID=user.PROFIL_ID LEFT JOIN civil_alerts ca ON ca.ID_ALERT=hs.ID_ALERT LEFT JOIN statut_validation sv ON sv.ID_STATUT=hs.STATUT_TRAITEMENT  WHERE 1 '.$condition1.' '.$date_interval.'';
    ...
    $order_column = array('obr.NUMERO_PLAQUE', 'DESCRIPTION_INFRATION', 'AMENDE', 'STATUT_PAYE', 'CIVIL_DESCRIPTION', 'NOM_UTILISATEUR');
    ...
    $search = !empty($_POST['search']['value']) ? ("AND obr.NUMERO_PLAQUE LIKE '%$var_search%' OR NOM_UTILISATEUR LIKE '%$var_search%' OR DESCRIPTION_INFRATION LIKE '%$var_search%' OR AMENDE LIKE '%$var_search%' ") : '';
    $critaire = '';
...
<li>
      ...
      <a hre='#' data-toggle='modal'
  data-target='#validations" . $row->ID_HISTO_SIGNALEMMENT  . "'><font color='green'>&nbsp;&nbsp;Valider</font></a>
     </li>
  ";
  ...
  data-target='#validations" . $row->ID_HISTO_SIGNALEMMENT  . "'><font color='red'>&nbsp;&nbsp;No Validé".$row->VALIDATION."</font></a>";
  ...
  <button class='btn btn-primary btn-md' data-dismiss='modal'>Quitter</button>
      ...
      $sub_array = array();
      $sub_array[] = '<input type="checkbox" class="data-check" value="'.$row->ID_HISTO_SIGNALEMMENT.'" onclick="afficher_btn_checkbox()"/>';
