...
class Identite extends CI_Controller
{
	...
	function index()
	{
		...
		$data['title'] = 'Listes des identités';
	...
	function listing()
	{
		...
		foreach ($fetch_peine as $row) {
		  ...
		  <div class='modal-footer'>
			...
			$img = '<img alt="Avtar" style="border-radius:50%;width:30px;height:30px" src="https://app.mediabox.bi/wasiliEate/uploads/personne.png">';

			if (!empty($row->PHOTO)) {
				$img = '<img alt="Avtar" style="border-radius:50%;width:30px;height:30px" src="' . $row->PHOTO . '">';
			}
			$sub_array[] = $img;
	...
	function update()
	{
		...
		$this->form_validation->set_rules('ID_AUTRES_CONTROLES', '', 'trim|required|callback_validate_name', array('required' => '<font style="color:red;size:2px;">Le champ est Obligatoire</font>'));
		...
		$this->form_validation->set_rules('NATIONNALITE_ID', '', 'trim|required|callback_validate_name', array('required' => '<font style="color:red;size:2px;">Le champ est Obligatoire</font>'));

		$this->form_validation->set_rules('SEXE', '', 'trim|required|callback_validate_name', array('required' => '<font style="color:red;size:2px;">Le champ est Obligatoire</font>'));
		...
		//$this->form_validation->set_rules('PHOTO_NOUVEAU','', 'trim|required|callback_validate_name',array('required'=>'<font style="color:red;size:2px;">Le champ est Obligatoire</font>'));
