...
class  Postes extends CI_Controller
{
	...
	function index()
	{
		...
		$this->load->view('Postes/Poste_List_View', $data);
	}
	...
	function listing()
	{
		...
		$i = 1;
		$query_principal = 'SELECT ID_POSTE , DESCRIPTION FROM postes WHERE 1';
		...
		$order_column = array('ID_POSTE', 'DESCRIPTION');
		...
		foreach ($fetch_infraction as $row) {
			...
			data-target='#mydelete" . $row->ID_POSTE  . "'><font color='red'>&nbsp;&nbsp;Supprimer</font></a></li>";
			$option .= "<li><a class='btn-md' href='" . base_url('ihm/Postes/getOne/' . $row->ID_POSTE ) . "'><label class='text-info'>&nbsp;&nbsp;Modifier</label></a></li>";
			$option .= " </ul>
			</div>
			<div class='modal fade' id='mydelete" .  $row->ID_POSTE  . "'>
			...
			<a class='btn btn-danger btn-md' href='" . base_url('ihm/Postes/delete/' . $row->ID_POSTE ) . "'>Supprimer</a>
	...
	function ajouter()
	{
		$data['title'] = 'Nouvelle poste';
		$this->load->view('Postes/Poste_Add_View', $data);
	}
	...
	function getOne($id)
	{
		$data['data'] = $this->Modele->getOne('postes', array('ID_POSTE' => $id));
		$data['title'] = 'Modification du poste';
		$this->load->view('Postes/Poste_Update_View', $data);
	}
