...
class Citoyen extends CI_Controller
{
	...
	function listing()
	{
		...
		$order_column = array('NOM_CITOYEN', 'PRENOM_CITOYEN', 'NOM_UTILISATEUR', 'NUMERO_CITOYEN', 'CNI');
		...
		foreach ($fetch_gerant as $row) {
			...
			$sub_array[] = '<table> <tbody><tr><td><img alt="Avtar" style="border-radius:50%;width:30px;height:30px" src="https://app.mediabox.bi/wasiliEate/uploads/personne.png"></td><td>' . $row->NOM_CITOYEN . ' ' . $row->PRENOM_CITOYEN . ' </td></tr></tbody></table>';
	...
	function add()
	{
		...
		$this->form_validation->set_rules('NOM_CITOYEN', '', 'trim|required|callback_validate_name', array('required' => '<font style="color:red;size:2px;">Le champ est Obligatoire</font>'));

		$this->form_validation->set_rules('PRENOM_CITOYEN', '', 'trim|required|callback_validate_name', array('required' => '<font style="color:red;size:2px;">Le champ est Obligatoire</font>'));

		$this->form_validation->set_rules('NUMERO_CITOYEN', '', 'trim|required|callback_validate_name', array('required' => '<font style="color:red;size:2px;">Le champ est Obligatoire</font>'));

		$this->form_validation->set_rules('MOT_DE_PASSE', '', 'trim|required|callback_validate_name', array('required' => '<font style="color:red;size:2px;">Le champ est Obligatoire</font>'));
	...
	function getOne($id)
	{
		...
		$data['title'] = "Modification de l'identité Citoyen";
