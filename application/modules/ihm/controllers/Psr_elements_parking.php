...
class Psr_elements_parking extends CI_Controller
{
   ...
   //       $critere_command = !empty($verifications) ? "and profils.PROFIL_ID = ".$verifications." " : "";
		...
		foreach ($fetch_psr as $row)
		{
			...
			$sub_array = array();
			$sub_array[] = '<table> <tbody><tr><td><a title="' . $source . '" href="#" onclick="get_imag(this.title)" ><img alt="Avtar" style="border-radius:50%;width:30px;height:30px" src="'.$source.'"></a></td><td>' . $row->NOM .' '.$row->PRENOM.' <font style="font-size:10px;color:red"><i class="far fa-comment-dots"></i></font></td></tr></tbody></table></a>';
