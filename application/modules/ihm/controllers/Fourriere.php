...
class Fourriere extends CI_Controller
{
      ...
      {
        if ($this->session->userdata('FOURRIERE') != 1) {
        ...
        foreach ($fetch_fourriere as $row) {
          $option = '<div class="dropdown ">
            ...
            $source = !empty($row->LOGO) ? $row->LOGO : "https://app.mediabox.bi/wasiliEate/uploads/personne.png";
             $sub_array[] = '<table> <tbody><tr><td><a title="' . $source . '" href="#" onclick="get_imag(this.title)" ><img alt="Avtar" style="border-radius:50%;width:30px;height:30px" src="' . $source . '"></a></td><td>' . $row->NOM_FOURRIERE .'</td></tr></tbody></table></a>';
            ...
            $sub_array[]="
                <a class = 'btn btn-success btn-sm 'href='#' id='".$row->NOM_FOURRIERE."'  title='".$row->NON_DU_RESPONSABLE."' onclick='getHistorique(".$row->ID_FOURRIERE.",this.title,this.id)' style='float:right'><span class = ''>".$this->get_nbre_intervation($row->ID_FOURRIERE)."</span></a>";
    ...
    function add(){
        ...
        $this->form_validation->set_rules('CARTE_ROSE', '', 'trim|required|callback_validate_name', array('required' => '<font style="color:red;size:2px;">Le champ est Obligatoire</font>'));

        $this->form_validation->set_rules('NOM_FOURRIERE', '', 'trim|required|callback_validate_name', array('required' => '<font style="color:red;size:2px;">Le champ est Obligatoire</font>'));

        $this->form_validation->set_rules('NON_DU_RESPONSABLE', '', 'trim|required|callback_validate_name', array('required' => '<font style="color:red;size:2px;">Le champ est Obligatoire</font>'));

        $this->form_validation->set_rules('EMAIL_FROURRIERE', '', 'trim|required|callback_validate_name', array('required' => '<font style="color:red;size:2px;">Le champ est Obligatoire</font>'));
        $this->form_validation->set_rules('TELEPHONE_FOURRIERE', '', 'trim|required|callback_validate_name', array('required' => '<font style="color:red;size:2px;">Le champ est Obligatoire</font>'));
        $this->form_validation->set_rules('TELEPHONE_FOURRIERE', '', 'trim|required|callback_validate_name', array('required' => '<font style="color:red;size:2px;">Le champ est Obligatoire</font>'));
...
redirect(base_url('ihm/Fourriere/'));
 ...
 $sms= '<div class="alert alert-success text-center" id="message">Carte rose introuvable</div>';
    ...
    //POUR AFFICHER  LES NBRE DE FOIS UNE FOURRIERE A INTERVENIR
    function get_nbre_intervation($ID_FOURRIERE){
        $nbre=$this->Model->getRequeteOne('SELECT COUNT(fo.ID_FOUR_HISTO) AS nbre  FROM psr_fourriere_historique fo  WHERE fo.ID_FOUR_STATUT>1 AND fo.ID_FOURRIERE='.$ID_FOURRIERE);
