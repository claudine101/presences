        ...
        if (empty($dateDay)) {
                ...
                if (!empty($rapport)) {
                        ...
                        foreach ($rapport as  $value) {
                                ...
                                $nbr = !empty($value['nbr']) ? $value['nbr'] : 0;
                        ...
                        for ($heures=0; $heures < 24 ; $heures++) { 
                            ...
                            $condu = date('d-m-Y',strtotime($dateDay)).' '.$hors;

                            $id = !empty($this->uri->segment(4)) ? $this->uri->segment(4) : 0;

                               $requete=  $this->Modele->getRequeteOne("SELECT SUM(`MONTANT`) AS AMANDE,count(*) as nbr, DATE_FORMAT(h.DATE_INSERTION, '%d-%m-%Y %H') FROM historiques h JOIN utilisateurs u ON u.ID_UTILISATEUR=h.ID_UTILISATEUR  WHERE u.PSR_ELEMENT_ID='".$id."' AND DATE_FORMAT(h.DATE_INSERTION, '%d-%m-%Y %H')='".$condu."' GROUP BY DATE_FORMAT(h.DATE_INSERTION, '%d-%m-%Y %H')");

                                $mm = !empty($requete['AMANDE']) ? $requete['AMANDE'] : 0;
                                $nbr = !empty($requete['nbr']) ? $requete['nbr'] : 0;
