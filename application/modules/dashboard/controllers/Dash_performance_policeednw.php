...
class Dash_performance_police extends CI_Controller
{
 ...
 $data['police']= $this->Model->getRequete("SELECT DISTINCT u.ID_UTILISATEUR as IDPO,concat(psr.NOM,' ',psr.PRENOM)  as name FROM utilisateurs u JOIN historiques h  ON u.ID_UTILISATEUR=h.ID_UTILISATEUR LEFT JOIN psr_elements psr ON psr.ID_PSR_ELEMENT=u.PSR_ELEMENT_ID  WHERE 1 AND u.PROFIL_ID=6 AND concat(psr.NOM,' ',psr.PRENOM) IS NOT NULL ORDER BY name ASC");
...
$idpol='';
  ...
  $idpol.=$KEY.',';
    ...
    // $long = strlen($idpol)-1;   
     $long =  substr($idpol,0,-1);
   $cond.= " AND  h.ID_UTILISATEUR IN (".$long.")"; 
      }
