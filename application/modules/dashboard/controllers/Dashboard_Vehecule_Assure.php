...
if ($this->session->userdata('PROFIL_ID') == 2) {
      $ID_ASSUREUR .= ' and ID_ASSUREUR='.$this->session->userdata('ID_INSTITUTION');
}

$assurence=$this->Model->getRequete("SELECT `ID_ASSUREUR`,`ASSURANCE` FROM `assureur` WHERE 1 ".
$ID_ASSUREUR." ORDER by ASSURANCE DESC");
...
if ($this->session->userdata('PROFIL_ID') == 2) {
      $ID_ASSUREUR .= $this->session->userdata('ID_INSTITUTION');
}else if(!empty($ID_ASSUREUR)){
      $ID_ASSUREUR .= $this->input->post('ID_ASSUREUR');;
}
...
$(\"#titre\").html(this.keyed+\" : Détails de toutes les assurances \");
}else if(this.key2==2){
$(\"#titre\").html(this.keyed+\" : Détails des assurances valables\");
}else{
 $(\"#titre\").html(this.keyed+\" : Détails des assurances expirées\");   
}
    ...
    ,
     {
      type: 'column',
        color: '#6610f2',
        name:'Assurances valables : (".number_format($immacat_valable,0,',',' ').")',
        borderColor:\"#f00000\",
...
$dateref=date('Y');
$dattes=$this->Model->getRequete("SELECT DISTINCT date_format(s.DATE_DEBUT,'%Y') AS mois FROM assurances_vehicules s WHERE 1 AND  DATE_DEBUT  IS NOT NULL AND date_format(s.DATE_DEBUT,'%Y') <=".$dateref." ".$criteres2."  ORDER BY  mois DESC");
