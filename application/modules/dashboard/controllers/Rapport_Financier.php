...
class Rapport_Financier extends CI_Controller
{
          ...
          function index()
          {
                   ...
                   $criteres_signal = " and (TIMESTAMPDIFF(day, `DATE_INSERT`,CURRENT_TIMESTAMP) =0)";
                    $criteres_histo = " and (TIMESTAMPDIFF(day, `DATE_INSERTION`,CURRENT_TIMESTAMP) =0)";
                    $titre="du  ".strftime('%d-%m-%Y',strtotime(date('Y-m-d')))." par heure";
                    ...
                    $titre="en  ".strftime('%Y-%m',strtotime($MOIS))." par jour ";
...
if($MOIS==''.$ANNE.'-01' || $MOIS==''.$ANNE.'-03' || $MOIS==''.$ANNE.'-05' || $MOIS==''.$ANNE.'-07' || $MOIS==''.$ANNE.'-08' || $MOIS==''.$ANNE.'-10' || $MOIS==''.$ANNE.'-12'){
    $ego=31;
}elseif($MOIS==''.$ANNE.'-04' || $MOIS==''.$ANNE.'-06' || $MOIS==''.$ANNE.'-09' || $MOIS==''.$ANNE.'-11' ){
    $ego=30;
}elseif($MOIS==''.$ANNE.'-02'){
    $ego=28;
}
...
$ego1=$MOIS.'-'.$ego;
        ...
        $coloneHisto = "DATE_FORMAT(`DATE_INSERTION`,'%H')";
        $coloneSignalement = "DATE_FORMAT(`DATE_INSERT`,'%H')";
        $titre="du  ".strftime('%d-%m-%Y',strtotime($DATE_UNE))." par heure ";
          ...
          if (!empty($DATE_UNE) && !empty($DATE_DEUX)) {
                       ...
                       $criteres_histo = " and DATE_FORMAT(DATE_INSERTION, '%Y-%m-%d') between  '" . $DATE_UNE."'  AND  '" . $DATE_DEUX."'";
                        ...
                        $coloneHisto = "DATE_FORMAT(`DATE_INSERTION`,'%d-%m-%Y')";
                        $coloneSignalement = "DATE_FORMAT(`DATE_INSERT`,'%d-%m-%Y')";
                        $titre="du  ".strftime('%d-%m-%Y',strtotime($DATE_UNE))." au  ".strftime('%d-%m-%Y',strtotime($DATE_DEUX))." ";
