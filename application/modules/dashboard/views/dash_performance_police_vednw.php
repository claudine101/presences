...
<div class="form-group col-md-2">
 <label>Police</label>
 <!-- <select class="form-control selectpicker" data-show-subtext="false" data-live-search="true" onchange="submit()" name="IDPO" id="IDPO"> -->
  <select name="IDPO[]" id="IDPO" class="selectpicker form-control" multiple data-live-search="true" data-actions-box="true" id="IDPO" >
<option value="">Sélectionner</option>
<?php
foreach ($police as $value){
if ($value['IDPO'] == $IDPO){?>
<option value="<?=$value['IDPO']?>" selected><?=$value['name']?></option>
<?php } else{ ?>
<option value="<?=$value['IDPO']?>" ><?=$value['name']?></option>
 <?php } } ?>
</select>
 <div class="col-md-1 col-sm-1 col-xs-1">
 <button type="button" onclick="save_data()" class="btn btn-info btn-block">Enregistrer</button>
                                   </div> 
</div>
