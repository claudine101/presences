...
class Psr_institution_user extends CI_Controller
{
     ...
     function index()
     {
          $data['institutions'] = $this->Modele->getRequete('SELECT ID_PSR_INSTITUTION, NOM_INSTITUTION FROM psr_institution WHERE 1 order by NOM_INSTITUTION ASC');
          $statuts = array(
              "Activer" => "1",
              "Désactiver" => "2");
          $data['statuts']=$statuts;

          $data['title'] = "Utilisateur des institutions";
          $this->load->view('institution/Utilisateurs_List_View', $data);
     }
     ...
     function listing()
     {
          ...
          if (!empty($STATUT)){
               if ($STATUT==2)
               {
                ...
                $criteres_statut=" AND user.IS_ACTIF=".$STATUT;
                if ($STATUT==0)
               {
                $STATUT=2;
               }
              }
          if (!empty($ID_PSR_INSTITUTION)) {
               $condition .=  ' and user.ID_INSTITUTION = "' . $ID_PSR_INSTITUTION . '"';
          }
          $query_principal = "SELECT CNI,ICON_LOGO,pr.CODE_PROFIL,ID_INSTITUTION,ID_UTILISATEUR, NOM_UTILISATEUR, user.PROFIL_ID,pr.STATUT, DATE_INSERTION ,user.IS_ACTIF AS ACTIF FROM utilisateurs user LEFT JOIN psr_elements psr on psr.ID_PSR_ELEMENT=user.PSR_ELEMENT_ID LEFT JOIN profil pr on pr.PROFIL_ID=user.PROFIL_ID WHERE user.PROFIL_ID=7 AND user.ID_INSTITUTION!='' "  . $condition . ' '.$criteres_statut;
     ...
     function ajouter()
     {
          ...
          $this->load->view('institution/Utilisateurs_Add_View', $data);
     }
     ...
     function getOne($id = 0)
     {
          ...
          $this->load->view('institution/Utilisateurs_Update_View', $data);
     }
