...
class Declaration_Vol extends CI_Controller
{
	...
	function index()
	{
		$data['title'] = 'Véhicules déclaré voler';
	...
	function listing()
	{
		...
		$order_column = array('NUMERO_PLAQUE', 'NOM_DECLARANT', 'PRENOM_DECLARANT','COULEUR_VOITURE', 'MARQUE_VOITURE', 'DATE_VOLER','STATUT');
	...
	function ajouter()
	{
		...
		$data['title'] = 'Déclaration d\'un vol';
		$this->load->view('declarations_add_v', $data);
	}
 //POUR  RECUPERER LES PROPRIETES  ASSOCIER POUR  UN PLAQUE
    function get_concerne(){
    	  $NUMERO_PLAQUE = $this->input->post('NUMERO_PLAQUE');
    	// $NUMERO_PLAQUE = "K2893A";
        
    	 // initialize cURL
			$curl = curl_init();

			// set the URL to fetch
			curl_setopt($curl, CURLOPT_URL, "http://app.mediabox.bi:2522/plaques/search?plaque=".$NUMERO_PLAQUE);
			...
			$table="<table id='concerne' class='table table-bordered table-striped table-hover table-condensed' style='width: 100%;'>
                    
                     <thead> 
                        <tr>
                          <th>NUMERO IDENTITE</th> 
                          <th>PROPRIETAIRE</th> 
                          <th>TELEPHONE</th> 
                          <th>NUMERO PLAQUE</th>
                          <th>COULEUR</th> 
                      ...
                      <tbody> 
                        <tr>
                          ...
                          <td>".$result->immatriculation->NUMERO_IDENTITE."</td>
                          <td>".$result->immatriculation->NOM_PROPRIETAIRE."</td>
                          <td>".$result->immatriculation->TELEPHONE."</td> 
                          <td><a title='".$result->result->numero_plaque."' onclick='show_vehicule(this.title)' class='btn btn-md dt-button btn-sm' href='#'>" .$result->result->numero_plaque. "</a></td> 
                          <td>".$result->immatriculation->COULEUR."</td> 
                          <td>".$result->result->numero_chassis."</td> 
                          <td>".$result->result->marque_voiture."</td> 
                          <td>".$result->immatriculation->MODELE_VOITURE."</td>
                          <td>".$result->immatriculation->NUMERO_CARTE_ROSE."</td>
                       </tr>
                      </tbody>
                     </table>";
...
// POUR  VERIFIER SI  LA VALEUR D'UN POUR CONTIENT LE CARACTERE ' "' 
	...
	function add()
	{
		...
		$this->form_validation->set_rules('NOM_DECLARANT', '', 'trim|required|callback_validate_name', array('required' => '<font style="color:red;size:2px;">Le champ est Obligatoire</font>'));
		$this->form_validation->set_rules('PRENOM_DECLARANT', '', 'trim|required|callback_validate_name', array('required' => '<font style="color:red;size:2px;">Le champ est Obligatoire</font>'));
		$this->form_validation->set_rules('COULEUR_VOITURE', '', 'trim|required|callback_validate_name', array('required' => '<font style="color:red;size:2px;">Le champ est Obligatoire</font>'));
		...
		$this->form_validation->set_rules('DATE_VOLER', '', 'trim|required|callback_validate_name', array('required' => '<font style="color:red;size:2px;">Le champ est Obligatoire</font>'));
    ...
    //print_r($highestRow);die();
        ...
        for($row=2; $row<=$highestRow; $row++)
        {
          ...
          $NOM_DECLARANT=$worksheet->getCellByColumnAndRow(1, $row)->getValue();
          $PRENOM_DECLARANT=$worksheet->getCellByColumnAndRow(4, $row)->getValue();
          $COULEUR_VOITURE=$worksheet->getCellByColumnAndRow(5, $row)->getValue();
          $MARQUE_VOITURE=$worksheet->getCellByColumnAndRow(6, $row)->getValue();
          $DATE_VOLER=$worksheet->getCellByColumnAndRow(2, $row)->getValue();
          $DATE_VOLER = PHPExcel_Style_NumberFormat::toFormattedString($DATE_DEBUT, 'YYYY-MM-DD');

          
				
          $data_insert = array(
				'NUMERO_PLAQUE' => trim($NUMERO_PLAQUE),
				'NOM_DECLARANT' => trim($NOM_DECLARANT),
				'PRENOM_DECLARANT' => trim($PRENOM_DECLARANT),
				'COULEUR_VOITURE' => trim($COULEUR_VOITURE),
				'MARQUE_VOITURE' => trim($MARQUE_VOITURE),
				'DATE_VOLER' => trim($DATE_VOLER),
			);
