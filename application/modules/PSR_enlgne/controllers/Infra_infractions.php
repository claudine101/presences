...
class  Infra_infractions extends CI_Controller
{
	...
	function index()
	{
		...
		$data['title'] = 'Listes des infractions';
	...
	function listing()
	{
		...
		$order_column = array('NIVEAU_ALERTE', 'VERIFICATION');
		...
		foreach ($fetch_infraction as $row) {
			...
			$done = $this->Modele->getOne('infra_infractions', array('ID_INFRA_INFRACTION' => $row->ID_INFRA_INFRACTION));
			$array = json_decode($done['NIVEAU_ALERTE'], true);
			// print_r($array['FR']);
			// exit();
			...
			$sub_array = array();
			//$sub_array[]=$i++;
			$sub_array[] = $array ? $array['ENG'] : $row->NIVEAU_ALERTE;
			//$row->NIVEAU_ALERTE;
	...
	function ajouter()
	{
		...
		$this->load->view('infractions_add_view', $data);
	}
	...
	function add()
	{
		...
		$this->form_validation->set_rules('NIVEAU_ALERTE', '', 'trim|required|callback_validate_name', array('required' => '<font style="color:red;size:2px;">Le champ est Obligatoire</font>'));
		$this->form_validation->set_rules('ID_TYPE_VERIFICATION', '', 'trim|required|callback_validate_name', array('required' => '<font style="color:red;size:2px;">Le champ est Obligatoire</font>'));
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$data_insert = array(
				...
				'INFRACTION_TRADUCTION' => $this->input->post('INFRACTION_TRADUCTION'),
	...
	function update()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$data_update = array(
			'NIVEAU_ALERTE' => $this->input->post('NIVEAU_ALERTE'),
			'INFRACTION_TRADUCTION' => $this->input->post(
			'INFRACTION_TRADUCTION'),
			'ID_TYPE_VERIFICATION' => $this->input->post('ID_TYPE_VERIFICATION'),
			);
