...
class Psr_institution extends CI_Controller
{
	...
	function get_chauffeur($id){
		...
		// $nbreFormat =  !empty($nbre) ? number_format(floatval($chauffeur['nbre']), 0, ',', ' ') : 0;
	...
	function get_vehicule($id){
		...
		// $nbreFormat =  !empty($nbre) ? number_format(floatval($vehicule['nbre']), 0, ',', ' ') : 0;
...
//function supprimer
	...
	{
		$query_principal= 'SELECT cp.NUMERO_PERMIS,cp.NOM_PROPRIETAIRE,cp.TELEPHONE, cp.SEXE,ip.NUMERO_PLAQUE,cp.POINTS FROM utilisateur_permis up LEFT  JOIN chauffeur_permis cp ON cp.ID_PERMIS=up.ID_PERMIS LEFT JOIN psr_institution pi ON pi.ID_PSR_INSTITUTION=up.ID_PSR_INSTITUTION  LEFT JOIN immatriculation_permis ip ON ip.ID_IMMATRICULATION_PERMIS=cp.ID_PERMIS WHERE 1 AND pi.ID_PSR_INSTITUTION='.$id ;

		$var_search = !empty($_POST['search']['value']) ? $_POST['search']['value'] : null;
		$var_search=str_replace("'", "\'", $var_search);
		$critaire='';

		$search = !empty($_POST['search']['value']) ? (" AND (cp.NUMERO_PERMIS LIKE '%$var_search%'or cp.NOM_PROPRIETAIRE LIKE '%$var_search%'or cp.POINTS LIKE '%$var_search%'or cp.SEXE LIKE'%$var_search%'or ip.NUMERO_PLAQUE LIKE '%$var_search%'or ip.TELEPHONE LIKE '%$var_search%' ) ") :'';
	...
	//fonction pour lister les véhicule d'un institution

	function vehicule($id)
	{ 
		$query_principal = 'SELECT oiv.NUMERO_PLAQUE,oiv.NOM_PROPRIETAIRE,oiv.NUMERO_IDENTITE,oiv.EMAIL,oiv.TELEPHONE FROM  utilisateur_vehicule uc LEFT JOIN utilisateurs u ON u.ID_UTILISATEUR=uc.ID_UTILISATEUR LEFT JOIN psr_institution pi ON pi.ID_PSR_INSTITUTION=u.ID_INSTITUTION   LEFT JOIN obr_immatriculations_voitures oiv ON oiv.ID_IMMATRICULATION=uc.ID_IMMATRICULATION WHERE 1 AND pi.ID_PSR_INSTITUTION='.$id ;

		$var_search = !empty($_POST['search']['value']) ? $_POST['search']['value'] : null;
		$var_search=str_replace("'", "\'", $var_search);
		$critaire='';

		$search = !empty($_POST['search']['value']) ? (" AND (oiv.NUMERO_PLAQUE LIKE '%$var_search%'or NOM_PROPRIETAIRE LIKE '%$var_search%'or NUMERO_IDENTITE LIKE '%$var_search%'or EMAIL LIKE'%$var_search%'or TELEPHONE LIKE '%$var_search%') ") :'';
		$query_secondaire=$query_principal.'  '.$critaire.' '.$search;
		$query_filter=$query_principal.'  '.$critaire;
		$fetch_cov_frais = $this->Model->datatable($query_secondaire);
		$data = array();
		...
		foreach($fetch_cov_frais as $info)
		{
			$post=array(); 
			$post[]=$info->NUMERO_PLAQUE;
			$post[]=$info->NOM_PROPRIETAIRE;
			$post[]=$info->NUMERO_IDENTITE;
			$post[]=$info->EMAIL;
			$post[]=$info->TELEPHONE;
			$data[] = $post;
    ...
    if(!empty($NUMERO_PLAQUE) ){
		...
		echo json_encode($montantFormat);
	}
	//fonction pour recuperer amendes totaux dune 'institution  
