...
class Historique extends CI_Controller
{
	...
	{
		if ($this->session->userdata('VERIFICATION') != 1) {
   ...
   function getIdentite($id_identite=0){

   	$id_identite = !empty($id_identite) ? $id_identite : 0;
   	
   	$data_identite=$this->Modele->getRequete('SELECT ID_IDENTITE, acr.ID_AUTRES_CONTROLES,acq.NEED_IDENTITE,acq.ID_CONTROLES_QUESTIONNAIRES,au.ID_AUTRES_CONTROLES, NOM, PRENOM,c.CommonName, NATIONNALITE_ID, SEXE, CNI, TELEPHONE, DATE_NAISSANCE, PHOTO, DATE_INSERT FROM autres_contr_rep_identite acr LEFT JOIN autres_controles au ON au.ID_AUTRES_CONTROLES=acr.ID_AUTRES_CONTROLES LEFT JOIN autres_controles_questionnaires acq ON acq.ID_CONTROLES_QUESTIONNAIRES=au.ID_CONTROLES_QUESTIONNAIRES LEFT JOIN countries c ON c.COUNTRY_ID=acr.NATIONNALITE_ID  WHERE acr.ID_CONTROLES_QUESTIONNAIRES='.$id_identite);
      ...
      $htmlDetail="";
		...
		foreach ($data_identite as $key => $data_identite) {
		...
		<thead>
		<tr>
		<th><img alt='Avtar' style='border-radius:50%;width:30px;height:30px' src=' ". $data_identite['PHOTO'] . "'>  ".$data_identite['NOM']." ".$data_identite['PRENOM']."
		...
		<tbody>";
		$Age=$this->notifications->ago($data_identite['DATE_NAISSANCE'], date('Y-m-d'));
         
		$htmlDetail .="<tr>

          <td>Pays :".$data_identite['CommonName']." <hr/>  sexe :".$data_identite['SEXE']." <hr/>CNI :".$data_identite['CNI']." <hr/>Telephone :".$data_identite['TELEPHONE']." <hr/> Age :".$Age."</td>
      ...
      // code...
		...
		//$map = $this->load->view('detail_view_map', $data, TRUE);
		$output = array('views_detail' => $htmlDetail, 'id' => $id_identite);
		echo json_encode($output);
   }



   function getNombreConcerne($id_quest = 0){

   	$req = $this->Modele->getRequeteOne("SELECT COUNT(`ID_CONTROLES_QUESTIONNAIRES`) as nbr FROM `autres_contr_rep_identite` WHEre `ID_CONTROLES_QUESTIONNAIRES`=".$id_quest." GROUP BY `ID_AUTRES_CONTROLES` LIMIT 1 ");
	...
	function getDetailComport($id_control = 0)
	{
	 ...
	 $htmlDetail ='<div class="col-11">
                    <h5 class="font-weight-bold">Contrôle du comportement </h5>
                    <p style="margin-top: -0.5em" class="text-dark">'.$this->getIfraction($id_control).'</p>';
		$total = 0;
		foreach ($this->Modele->getRequete($dataDetail) as $key => $value) {
			...
			$total += $value['MONTANT'];
			$htmlDetail .= '<div class="row mt-2 d-flex align-items-center"><span style="background-color: lightgrey; border-radius:50%; width: 40px; height: 40px" class="p-1 mr-1 d-flex justify-content-center align-items-center"><i style="font-size: 20px" class="fa fa-money" aria-hidden="true"></i></span> <span>'.$value['INFRACTIONS']. $reponse. $description.' : <b>'. number_format($value['MONTANT'], 0, '.', ' ') .'</b></span></div><br>';

			// $htmlDetail .= '<div class="row mt-2 d-flex align-items-center"><span style="background-color: lightgrey; border-radius:50%; width: 40px; height: 40px" class="p-1 mr-1 d-flex justify-content-center align-items-center"><i style="font-size: 20px" class="fa fa-money" aria-hidden="true"></i></span> <span>'.$imagedata.'</b></span></div>';

			// $htmlDetail .= '<div class="row mt-2 d-flex align-items-center"><span style="background-color: lightgrey; border-radius:50%; width: 40px; height: 40px" class="p-1 mr-1 d-flex justify-content-center align-items-center"><i style="font-size: 20px" class="fa fa-money" aria-hidden="true"></i></span> <span>'.$optidentite.'</b></span></div>';
		}

		$htmlDetail .= '<div class="row mt-2 d-flex align-items-center"><span style="background-color: lightgrey; border-radius:50%; width: 40px; height: 40px" class="p-1 mr-1 d-flex justify-content-center align-items-center"><i style="font-size: 20px" class="fa fa-money" aria-hidden="true"></i></span> <span>Total : <b>'. number_format($total, 0, '.', ' ') .'</b></span></div>';
    ...
    $psr_element='';
     ...
     if (!empty($ID_PSR_ELEMENT)){
        $psr_element=' AND pe.ID_PSR_ELEMENT='.$ID_PSR_ELEMENT;
    }
     if (!empty($filtre)){
        $type_verif=' AND ID_HISTORIQUE_CATEGORIE='.$filtre;
    }
    if(!empty($DATE1) AND !empty($DATE2) )
  {
    
  $cond=" AND date_format(h.DATE_INSERTION,'%Y-%m-%d') BETWEEN '".$DATE1."' AND '".$DATE2."'" ;
  }
  if(empty($DATE1) AND !empty($DATE2) )
  {
  $cond=" AND date_format(h.DATE_INSERTION,'%Y-%m-%d')='".$DATE2."'";
   
  }
  if(!empty($DATE1) AND empty($DATE2))
  {

     $cond=" AND date_format(h.DATE_INSERTION,'%Y-%m-%d')='".$DATE1."'";
  }
		...
		(SELECT NIVEAU_ALERTE FROM infra_infractions WHERE ID_INFRA_INFRACTION= ID_ASSURANCE_PEINE) AS ASSURANCE,
		(SELECT NIVEAU_ALERTE FROM infra_infractions WHERE ID_INFRA_INFRACTION= ID_CONTROLE_TECHNIQUE_PEINE) AS CONTROL_TECHNIQUE,
		(SELECT NIVEAU_ALERTE FROM infra_infractions WHERE ID_INFRA_INFRACTION= ID_VOL_PEINE) AS VOL,(SELECT NIVEAU_ALERTE FROM infra_infractions WHERE ID_INFRA_INFRACTION= ID_PERMIS_PEINE) AS PERMIS_PEINE, h.NUMERO_PLAQUE ,ID_IMMATRICULATIO_PEINE,ID_ASSURANCE_PEINE,ID_CONTROLE_TECHNIQUE_PEINE,ID_VOL_PEINE,ID_PERMIS_PEINE, NUMERO_PERMIS, concat(pe.NOM," ",pe.PRENOM) as user,pe.NUMERO_MATRICULE,pe.ID_PSR_ELEMENT, h.LATITUDE, h.LONGITUDE,h.DATE_INSERTION,h.MONTANT,h.IS_PAID,ID_CONTROLE_EQUIPEMENT,ID_SIGNALEMENT FROM historiques h LEFT JOIN historiques_categories hc ON h.ID_HISTORIQUE_CATEGORIE=hc.ID_CATEGORIE LEFT JOIN utilisateurs us ON us.ID_UTILISATEUR=h.ID_UTILISATEUR  LEFT JOIN psr_elements pe ON pe.ID_PSR_ELEMENT = us.PSR_ELEMENT_ID WHERE 1 '.$type_verif.$critere_date.$cond.$psr_element."";
      ...
      $var_search = !empty($_POST['search']['value']) ? $_POST['search']['value'] : null;
		...
		// $search = !empty($_POST['search']['value']) ? ("AND NUMERO_PLAQUE LIKE '%".$var_search."%' AND pe.PRENOM LIKE '%".$var_search."%' AND pe.NOM LIKE '%".$var_search."%' ") : '';
		$search = !empty($_POST['search']['value']) ? ("AND pe.NOM LIKE '%$var_search%'
			 OR pe.PRENOM LIKE '%$var_search%' OR hc.DESCRIPTION LIKE '%$var_search%' OR h.RAISON_ANNULATION  OR h.NUMERO_PLAQUE LIKE '%$var_search%' OR h.NUMERO_PERMIS LIKE '%$var_search%'  OR h.MONTANT LIKE '%$var_search%'") : '';
	...
	function getInfraImmatri($id_immatri = 0)
	{
		...
		$htmlDetail ='<div class="col-11">
                      <h5 class="font-weight-bold"> Immatriculation</h5>
                      <p style="margin-top: -0.5em" class="text-dark">'.$this->getIfraction($id_immatri).'</p>';
		$total = 0;
		foreach ($this->Modele->getRequete($dataDetail) as $key => $value) {

			$total += $value['MONTANT'];
			$htmlDetail .= '<div class="row mt-2 d-flex align-items-center"><span style="background-color: lightgrey; border-radius:50%; width: 40px; height: 40px" class="p-1 mr-1 d-flex justify-content-center align-items-center"><i style="font-size: 20px" class="fa fa-money" aria-hidden="true"></i></span> <span>'. $value['AMENDES'] .' : <b>'. number_format($value['MONTANT'], 0, '.', ' ') .'</b></span></div>';
		}
	...
	function getInfrassur($id_immatri = 0)
	{
		...
		$htmlDetail ='<div class="col-11">
                      <h5 class="font-weight-bold">Contrôle Assurance </h5>
                      <p style="margin-top: -0.5em" class="text-dark">'.$this->getIfraction($id_immatri).'</p>';
		$total = 0;
		foreach ($this->Modele->getRequete($dataDetail) as $key => $value) {
	...
	function getInfraPermis($id_immatri = 0)
	{
		...
		$htmlDetail ='<div class="col-11">
                      <h5 class="font-weight-bold">Permis de conduire</h5>
                      <p style="margin-top: -0.5em" class="text-dark">'.$this->getIfraction($id_immatri).'</p>';
		$total = 0;
		foreach ($this->Modele->getRequete($dataDetail) as $key => $value) {
	...
	function getInfraVol($id_immatri = 0)
	{
        ...
        $htmlDetail ='<div class="col-11">
                      <h5 class="font-weight-bold">Police judiciaire</h5>
                      <p style="margin-top: -0.5em" class="text-dark">'.$this->getIfraction($id_immatri).'</p>';
		$total = 0;
		foreach ($this->Modele->getRequete($dataDetail) as $key => $value) {
	...
	function getInfracontroleTechnique($technique)
	{
		...
		$htmlDetail ='<div class="col-11">
                      <h5 class="font-weight-bold"> Contrôle Technique</h5>
                      <p style="margin-top: -0.5em" class="text-dark">'.$this->getIfraction($technique).'</p>';
		$total = 0;
		foreach ($this->Modele->getRequete($dataDetail) as $key => $value) {
