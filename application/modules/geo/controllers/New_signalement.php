...
class New_signalement extends CI_Controller
{
  ...
  //FUNCRION UTILISE SI LE SIGNALEMENT  N'EST PAS SUR STATUT ZERO  
  public function control_rapide()
  { 
      $NUMERO_PLAQUE = $this->input->post('NUMERO_PLAQUE');
      $ID_SIGNALEMENT_NEW = $this->input->post('ID_SIGNALEMENT_NEW');
      $MONTANT_AMANDE = $this->input->post('MONTANT_AMANDE');
  ...
  function get_detail_signal($id = null){
    ...
    $historique =  $this->Modele->getRequeteOne('SELECT ID_SIGNALEMENT  FROM historiques  WHERE ID_SIGNALEMENT= '.$id);
    ...
    // $histo='<input type="hidden" id="histo" name="histo" value="0">';
    // if(!empty($historique)){
        // $histo='<input type="text" id="histo" name="histo" value="' .$historique['ID_SIGNALEMENT']. '">';
    // }
    ...
    $titre = '<div class="profile mr-2"><h4 class="mt-0 mb-0 text-left">Auteur ' . $sex . ' ' . $nom . ' </h4>
      ...
      if ($vehicule['ID_ALERT_STATUT']==0 ) {
        ...
        if(empty($historique)){
          ...
          $btn3 = "<button id='idbutton' type='button' id='' onclick='On_Save()' class='btn btn-primary btn-md' >Voir les documents</button>";
        }
        else{
    ...
    $donnes =  array('historique'=>$historique,'titre' => $titre, 'donne' => $datas, 'btn_data'=>$btn_data);
