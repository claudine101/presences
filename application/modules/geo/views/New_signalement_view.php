...
function On_Save(){
  if ($('#plaque_numbers').val() == "") {
     ...
     Swal.fire({
      title: "Souhaitez-vous effectuer un contrôle rapide avant  d\'afficher les documents sur le plaque numéro "+NUMERO_PLAQUE+ "?",
      showDenyButton: true,
      confirmButtonText: 'Maintenant',
      denyButtonText: `Pas maintenant`,
    }).then((result) => {
      ...
      /* Debut ajax*/
         $.ajax({
          url : "<?=base_url()?>geo/New_signalement/control_rapide/",
          type : "POST",
          dataType: "JSON",
          cache:false,
          data: {
