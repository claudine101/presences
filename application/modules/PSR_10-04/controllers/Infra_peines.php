...
class Infra_peines extends CI_Controller
{
  ...
  {
    //if()
    $type_verification = $this->Modele->getRequete(' SELECT ID_TYPE_VERIFICATION, VERIFICATION FROM type_verification WHERE 1 ORDER BY VERIFICATION ASC');

    $data['type_verification'] = $type_verification;
    $data['title'] = "Pénalités";
  ...
  function listing()
  {
    ...
    $order_column = array('NIVEAU_ALERTE', 'AMENDES', 'MONTANT', 'POINTS','APPEL_FOURRIERE');
    ...
    <button class='btn btn-primary btn-md' data-dismiss='modal'>Quitter</button>
      ...
      $fourrs = !empty($row->APPEL_FOURRIERE == 1) ? 'check' : 'ban';
      $fourrs_btn = !empty($row->APPEL_FOURRIERE == 0) ? ' btn-danger' : 'btn-success';
      // $sub_array[]=$i++;
      ...
      $sub_array[] = '<center><a href="#" class="btn '.$fourrs_btn.' btn-sm"><i title="Fourrière" class="fa fa-'.$fourrs.'" aria-hidden="true"></i></a></center>';
  ...
  function ajouter()
  {
    $data['infraPeines'] = $this->Modele->getRequete('SELECT ID_INFRA_INFRACTION, NIVEAU_ALERTE FROM infra_infractions WHERE 1 ORDER BY NIVEAU_ALERTE ASC ');

    $data['title'] = 'Pénalité sur l\'infraction';
  ...
  function add()
  {
    ...
    $this->form_validation->set_rules('ID_INFRA_INFRACTION', '', 'trim|required|callback_validate_name', array('required' => '<font style="color:red;size:2px;">Le champ est Obligatoire</font>'));


    $this->form_validation->set_rules('AMENDES', '', 'trim|required|callback_validate_name', array('required' => '<font style="color:red;size:2px;">Le champ est Obligatoire</font>'));
    ...
    $this->form_validation->set_rules('POINTS', '', 'trim|required|callback_validate_name', array('required' => '<font style="color:red;size:2px;">Le champ est Obligatoire</font>'));
    ...
    if ($this->form_validation->run() == FALSE) {
      ...
      $data_insert = array(
        ...
        'APPEL_FOURRIERE'   => $this->input->post('APPEL_FOURRIERE'),
      );

      $table = 'infra_peines';
      $this->Modele->create($table, $data_insert);

      $data['message'] = '<div class="alert alert-success text-center" id="message">' . "Enregistrement faite avec succès" . '</div>';
      $this->session->set_flashdata($data);
      redirect(base_url('PSR/Infra_peines/ajouter'));
    }
  }
  function getOne($id)
  {
    ...
    $data['title'] = "Modification d'une pénalité";
