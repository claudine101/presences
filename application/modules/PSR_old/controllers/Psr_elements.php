...
class Psr_elements extends CI_Controller
{
	...
	function listing()
	{
		...
		$i = 1;
		$query_principal = 'SELECT psr.DATE_INSERT,IF(psr.SEXE = "H","mr","md") as SEXE,psr.DATE_INSERT,psr_grade.GRADE,DATE_NAISSANCE,ID_PSR_ELEMENT, NOM, PRENOM, NUMERO_MATRICULE,sp.PROVINCE_NAME AS province, sc.COMMUNE_NAME AS commune ,sz.ZONE_NAME AS zone, sco.COLLINE_NAME AS Colline, TELEPHONE, EMAIL,PHOTO, psr.LATITUDE, psr.LONGITUDE, psr.IS_ACTIF, psr.ID_GRADE, p.STATUT FROM psr_elements psr LEFT JOIN syst_provinces sp ON psr.PROVINCE_ID=sp.PROVINCE_ID LEFT JOIN syst_communes sc ON sc.COMMUNE_ID=psr.COMMUNE_ID LEFT JOIN syst_zones sz on sz.ZONE_ID=psr.ZONE_ID LEFT JOIN syst_collines sco ON sco.COLLINE_ID=psr.COLLINE_ID LEFT JOIN utilisateurs us on us.PSR_ELEMENT_ID=psr.ID_PSR_ELEMENT LEFT JOIN profil p ON p.PROFIL_ID = us.PROFIL_ID LEFT JOIN psr_grade ON psr.ID_GRADE=psr_grade.ID_GRADE WHERE 1  ';
		...
		$order_column = array('NOM', 'PRENOM', 'NUMERO_MATRICULE', 'GRADE', 'province', 'commune', 'zone', 'Colline', 'TELEPHONE', 'STATUT','DATE_INSERT');
    ...
    function validate_name($name)
	 ...
	 {
	          $email= $this->Modele->getOne('utilisateurs', array('NOM_UTILISATEUR' => $name));
	           if (!empty($email)) {
	             $this->form_validation->set_message("validate_email","L'email existée");
	            return FALSE;
	           }
	           else{
	                return TRUE;
	...
	function add()
	{
		...
		$this->form_validation->set_rules('EMAIL', '', 'trim|required|callback_validate_name|callback_validate_email', array('required' => '<font style="color:red;size:2px;">Le champ est Obligatoire</font>'));
	...
	function update()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->getOne($this->input->post('ID_PSR_ELEMENT'));
		} else {
