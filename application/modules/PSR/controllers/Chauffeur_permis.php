...
class Chauffeur_permis extends CI_Controller
{
	...
	public function controle_permis()
	{ 
	    $numeroPermis = $this->input->post('NUMERO_PERMIS');
	    $lat = "-3.45298";
	    $long = "29.68734";
	    $donne = array( 'numeroPermis'  =>  $numeroPermis,
	                    'lat'           =>  $lat,
	                    'long'          =>  $long,
	                    'controleType'  =>  1
	                     );
	     $data = $this->notifications->control_permis($donne);
	...
	function listing()
	{
		...
		$query_principal = "SELECT cp.POINTS, cp.ID_PERMIS, cp.NUMERO_PERMIS, cp.NOM_PROPRIETAIRE, cp.CATEGORIES, cp.DATE_NAISSANCE, cp.DATE_DELIVER, cp.DATE_EXPIRATION,cp.TELEPHONE,cp.LOGO_PROFIL,pi.NOM_INSTITUTION FROM chauffeur_permis cp LEFT  JOIN  psr_institution pi ON pi.ID_PSR_INSTITUTION=cp.ID_PSR_INSTITUTION
