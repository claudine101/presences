<?php
/**
 * Auteur NDAYISISABA Claudine
 * email prof:claudine@mediabox.bi
 * En date du 10/03/2023 a partir de 8h
 * CRUD TRANSPORT
 **/
class Transport extends CI_Controller
{
	...
	{
		if ($this->session->userdata('TRANSPORT') != 1 && $this->session->userdata('PSR_ELEMENT') != 1) {
			redirect(base_url());
		}
	}
	function index()
	{
		$data['title'] = 'Permis de transport';
		$this->load->view('transport/transport_list_view', $data);
	}

	function listing()
	{
		$i = 1;
		$query_principal = 'SELECT otraco.TELEPHONE, otraco.ID_CONTROLE, otraco.NUMERO_CONTROLE , otraco.NUMERO_PLAQUE,obr.ID_IMMATRICULATION, otraco.NUMERO_CHASSIS,otraco.PROPRIETAIRE ,otraco.DATE_DEBUT,otraco. DATE_VALIDITE,otraco.TYPE_VEHICULE  FROM otraco_transports otraco LEFT JOIN obr_immatriculations_voitures obr ON obr.NUMERO_PLAQUE=otraco.NUMERO_PLAQUE WHERE 1';
		...
		$order_column = array('TYPE_VEHICULE', 'NUMERO_CONTROLE', 'NUMERO_PLAQUE', 'NUMERO_CHASSIS', 'PROPRIETAIRE','TELEPHONE', 'DATE_DEBUT', 'DATE_VALIDITE', 'TYPE_VEHICULE');
		...
		foreach ($fetch_psr as $row) {
			...
			$option .= "<li><a class='btn-md' href='" . base_url('PSR/Transport/getOne/' . $row->ID_CONTROLE) . "'><label class='text-info'>&nbsp;&nbsp;Modifier</label></a></li>";
			$option .= " </ul>
			</div>
			...
			<a class='btn btn-danger btn-md' href='" . base_url('PSR/Transport/delete/' . $row->ID_CONTROLE) . "'>Supprimer</a>
	...
	function ajouter()
	{
		...
		$this->load->view('transport/transport_add_view', $data);
	}
	...
	function add()
	{
		...
		$this->form_validation->set_rules('NUMERO_CONTROLE', '', 'trim|required|callback_validate_name', array('required' => '<font style="color:red;size:2px;">Le champ est Obligatoire</font>'));
		...
		$this->form_validation->set_rules('PROPRIETAIRE', '', 'trim|required|callback_validate_name', array('required' => '<font style="color:red;size:2px;">Le champ est Obligatoire</font>'));
      $this->form_validation->set_rules('TELEPHONE', '', 'trim|required|callback_validate_name', array('required' => '<font style="color:red;size:2px;">Le champ est Obligatoire</font>'));
		...
		$this->form_validation->set_rules('TYPE_VEHICULE', '', 'trim|required|callback_validate_name', array('required' => '<font style="color:red;size:2px;">Le champ est Obligatoire</font>'));
		if ($this->form_validation->run() == FALSE) {
			...
			$table = 'otraco_transports';

			$this->Modele->create($table, $data_insert);

			$data['message'] = '<div class="alert alert-success text-center" id="message">' . "L'ajout se faite avec succès" . '</div>';
			$this->session->set_flashdata($data);
			redirect(base_url('PSR/Transport/'));
		}
	}
	function getOne($id = 0)
	{
		$data['controles'] = $this->Modele->getOne('otraco_transports', array('ID_CONTROLE' => $id));
		$data['plaques'] = $this->Model->getRequete('SELECT ID_IMMATRICULATION, NUMERO_PLAQUE FROM obr_immatriculations_voitures WHERE 1 ORDER BY NUMERO_PLAQUE ASC');

		$data['title'] = "Modification d'un  controle technique";
		$this->load->view('transport/transport_update_view', $data);
	}
	...
	function update()
	{
      ...
      $this->form_validation->set_rules('TELEPHONE', '', 'trim|required|callback_validate_name', array('required' => '<font style="color:red;size:2px;">Le champ est Obligatoire</font>'));
		...
		$this->Modele->update('otraco_transports', array('ID_CONTROLE' => $id), $data_insert);
    ...
    redirect(base_url('PSR/Transport/index'));
